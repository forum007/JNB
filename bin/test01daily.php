...
//				printf("DATA無[%s][%s]\n", $row['ita_code'],$row['fund_name']);
				...
				$sql = "select standard_date, nav, tna, diff_last_day_yen, diff_last_day ";
				$sql .= "from lipper_price_daily ";
				...
				while($row3 = mysqli_fetch_assoc($res2)) {
					$ins = "replace into ms_daily values(";
					$ins .= "'" . $row['fund_code'] . "',";
					$ins .= "'" . $row3['standard_date'] . "',";
					$ins .= $row3['nav'] . ",";
					if(strlen($row3['tna']) == 0){
						$ins .= "null,";
					}else{
						$ins .= $row3['tna'] . ",";
					}
					if(strlen($row3['diff_last_day_yen']) == 0){
						$ins .= "null,";
					}else{
						$ins .= $row3['diff_last_day_yen'] . ",";
					}
					if(strlen($row3['diff_last_day']) == 0){
						$ins .= "null);";
					}else{
						$ins .= $row3['diff_last_day'] . ");";
					}
printf("%s\n", $ins);
